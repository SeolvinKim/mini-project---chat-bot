...
먼저 코드가 키워드로 분류를 시도하고(명확하면 즉시 응답), 모호할 때만 라우터 LLM이 최근 대화와 프로필까지 참고해 문맥을 분류하고 `그 자격증 일정도 알려줘` 같은 후속 질문을 독립된 요청으로 정리합니다. `OPENAI_API_KEY`가 없으면 키워드 기반 분류만으로 동작합니다.

## 모델 구조

질문 판단(라우팅)과 답변 생성을 분리하고, 역할별로 다른 모델을 씁니다.

| 단계 | 함수 (`core/llm.py`) | 모델 | 비고 |
|---|---|---|---|
| 라우팅·분류 | `get_router_llm()` | `gpt-5.4-mini` | 키워드로 안 될 때만 호출 |
| 콘텐츠 생성 | `get_generation_llm()` | `gpt-5.4` | 자소서·면접·직무 |
| RAG 임베딩 | `get_embeddings()` | `text-embedding-3-small` | |

`certificate`(자격증) Tool은 규칙기반이라 LLM을 쓰지 않습니다. 생성 Tool은 반드시 `get_generation_llm()`을 사용하세요(라우팅용 mini 모델로 생성 금지). 전체 흐름도는 `AGENTS.md` 참고.
